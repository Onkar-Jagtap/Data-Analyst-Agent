import { parseCleanNumber, isNullOrEmpty } from './profiler.js';
...
// Clean identifier (strip quotes: "Col Name" -> Col Name, `Col` -> Col)
...
// Parse SQL SELECT expressions (e.g. "Region", "SUM(Revenue) AS total_revenue", "ROUND(AVG(Margin), 2) as avg_m")
...
// Compute aggregate on a set of rows
function computeAgg(
  rows: Record<string, any>[],
...
/**
 * Execute SQL Query against an in-memory dataset
 */
export function executeSqlQuery(rawRows: Record<string, any>[], sqlQuery: string): SqlQueryResult {
  ...
  try {
    ...
    // If query contains CTEs: WITH ... AS (...), extract the final SELECT
    ...
    // Match main clauses: SELECT ... FROM ... [WHERE ...] [GROUP BY ...] [HAVING ...] [ORDER BY ...] [LIMIT ...]
    ...
    if (!match) {
      // Fallback: simpler select without FROM or with other syntax
      return {
        success: false,
        columns: [],
        rows: [],
    ...
    const havingStr = match[6] ? match[6].trim() : '';
    ...
    // 2. Handle GROUP BY or Global Aggregate
    ...
    if (groupByStr || hasAggregates) {
      const groupColNames = groupByStr
        ? groupByStr.split(',').map(s => {
            ...
            // Check if ordinal index: GROUP BY 1, 2
      ...
      // If global aggregate with 0 rows, emit 1 row of defaults
      ...
      // Plain SELECT without aggregation
      for (const row of filteredRows) {
        const resultRow: Record<string, any> = {};

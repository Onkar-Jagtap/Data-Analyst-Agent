  ...
  const crossDepartmentRisks: CrossDepartmentRisk[] = [
    {
      ...
      metricImpact: `-$${Math.round(returnDragDollars * 0.65).toLocaleString()} / yr in return write-offs`,
      evidence: 'Orders containing heavy hardware (Server Blade X9) shipped via UPS Freight exhibit a 24.8% return rate with transit damage, flipping net deal contribution margin negative.',
      recommendedIntervention: 'Require crating and transition heavy hardware contracts to specialized air-ride white-glove carriers (FedEx Priority or DHL Express).',
    },
    {
      id: 'risk-2',
      title: 'Meta Campaigns Lead Dilution & Sales Friction',
      severity: 'Warning',
      departments: ['Marketing', 'Leads', 'Sales'],
      metricImpact: '68% lower deal conversion vs LinkedIn B2B',
      evidence: 'Meta campaigns generate low cost-per-click traffic ($1.80 CPC) but average lead score is only 34/100, consuming sales rep follow-up capacity with low closed-won velocity.',
      recommendedIntervention: 'Reallocate 40% of Meta ad spend to LinkedIn B2B account-based marketing and Google High-Intent Search.',
    },
    {
      id: 'risk-3',
      ...
      evidence: 'Discretionary discounts average over 20% on selected enterprise accounts without contractual multi-year volume commitments or margin covenants.',
      recommendedIntervention: 'Implement a hard floor policy: discounts over 10% require VP of Sales sign-off, and discounts over 18% require automated CFO approval.',
    },
  ];
  ...
  const strategicActions: StrategicImprovementAction[] = [
    {
      ...
      finding: 'Sales reps are currently incentivized on top-line gross bookings, driving aggressive discounting and pushing high-return hardware SKUs that severely erode retained cash flow.',
      rootCause: 'Data silos between CRM bookings, warehouse return write-downs, and Finance COGS prevent sales incentives from reflecting actual delivered deal margin.',
      concreteAction: 'Establish an automated gross-to-net commission hurdle requiring a minimum 20% net contribution margin threshold before tier-1 commission payouts are unlocked.',
      expectedFinancialImpact: 'Eliminates negative-margin deal concessions and recovers +$145,000 in retained net margin annually.',
      impactMetric: '+$145,000 / yr',
      annualEbitdaImpact: 145000,
    ...
    },
    {
      ...
      finding: 'LinkedIn B2B campaigns deliver an outstanding Marketing Efficiency Ratio (4.8x) and 29.8% win rates, whereas Meta campaigns incur high bounce rates and low-intent registrations.',
      rootCause: 'Top-of-funnel marketing campaigns were historically evaluated on raw lead volume and cost-per-click rather than downstream deal close rates.',
      concreteAction: 'Shift $18,000/month in Meta ad spend into targeted LinkedIn Account-Based Marketing (ABM) and Google high-intent enterprise search campaigns.',
      expectedFinancialImpact: 'Reduces Blended CAC by $190 (-28%) from $680 to $490, accelerating pipeline velocity and lifting EBITDA by +$74,000 annually.',
      impactMetric: '+$74,000 / yr',
      annualEbitdaImpact: 74000,
    ...
    },
    {
      ...
      priority: 'Critical',
      timeframe: 'Immediate 30-Day',
      title: 'Enforce Automated Multi-Tier Discount Floor Guardrails',
      finding: 'Discretionary sales discounting averages over 20% on mid-market accounts, conceding substantial margin realization without securing multi-year contractual commitments.',
      rootCause: 'Sales reps possess unconstrained discounting authority up to 25% without requiring automated workflow triggers or finance sign-off.',
      concreteAction: 'Deploy a strict 3-tier deal desk matrix: up to 10% discretionary rep approval, 10%–18% Regional Director approval, and >18% mandatory CFO authorization.',
      expectedFinancialImpact: 'Recovers +$110,000 in baseline top-line price realization across quarterly sales cycles without harming close rates.',
      impactMetric: '+$110,000 / yr',
      annualEbitdaImpact: 110000,
    ...
    },
    {
      ...
      priority: 'Critical',
      timeframe: 'Immediate 30-Day',
      title: 'Remediate Freight Damage on Server Hardware Shipments',
      finding: 'Server Blade X9 hardware accounts for 58% of all operational returns, driven by heavy-freight transit shock and handling failures through UPS Freight.',
      rootCause: 'Heavy sensitive electronics are shipped in standard corrugated packaging without shock-watch indicators through general freight consolidation hubs.',
      concreteAction: 'Mandate MIL-SPEC custom foam packaging with drop sensors, and transition heavy server hardware routing exclusively to FedEx Priority Air-Ride freight.',
      expectedFinancialImpact: 'Slashes return rate from 24.8% to under 3.5%, reclaiming +$92,000 in reverse freight expense and inventory write-offs annually.',
      impactMetric: '+$92,000 / yr',
      annualEbitdaImpact: 92000,
    ...
    },
    {
      ...
      priority: 'High',
      timeframe: '60-90 Day',
      title: 'Automate B2B Bank Transfer Incentives & Standardize Net-60 Payables',
      finding: 'Credit card merchant interchange fees (2.9%) drain profitability on large B2B invoices, while vendor payment terms mismatch receivables collection cycles.',
      rootCause: 'Invoicing defaults to credit card processing on transactions over $5,000 without automated ACH or wire discounts, while vendor terms remain at net-30.',
      concreteAction: 'Provide a 1.5% prompt-pay discount for ACH/wire settlements on invoices >$3,000 (capping transaction fees at 0.3%) and renegotiate supplier terms to net-60.',
      expectedFinancialImpact: 'Generates +$28,000 in transaction fee savings and improves net working capital cash conversion cycles by 14 days.',
      impactMetric: '+$28,000 / yr',
      annualEbitdaImpact: 28000,

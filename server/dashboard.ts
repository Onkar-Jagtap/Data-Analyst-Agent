...
export function computeDashboardData(
  ...
  // 13. Advanced Business Calculations (Revenue, Profit Margin, AOV, Pareto, Run-Rate, Growth, Adjustments)
  const profitMarginPct = primarySum !== 0 ? Math.round(((secondarySum / primarySum) * 100) * 100) / 100 : 0;
  const aov = activeRowsCount > 0 ? primarySum / activeRowsCount : 0;
  const topSegment = dimensionBreakdown[0] || { category: 'N/A', share: 0, primarySum: 0 };

  // Pareto 80/20 analysis: share generated by top 20% of segments
  const top20Count = Math.max(1, Math.ceil(dimensionBreakdown.length * 0.2));
  const top20Sum = dimensionBreakdown.slice(0, top20Count).reduce((acc, d) => acc + d.primarySum, 0);
  const paretoTop20SharePct = primarySum > 0 ? Math.round(((top20Sum / primarySum) * 100) * 10) / 10 : 0;

  // Period-over-period growth calculation
  let periodGrowthPct: number | null = null;
  if (timeSeries.length >= 2) {
    const priorPeriodVal = timeSeries[timeSeries.length - 2].primarySum;
    const currentPeriodVal = timeSeries[timeSeries.length - 1].primarySum;
    if (priorPeriodVal !== 0) {
      periodGrowthPct = Math.round((((currentPeriodVal - priorPeriodVal) / Math.abs(priorPeriodVal)) * 100) * 10) / 10;
    }
  }

  // Negative adjustment / returns / refunds calculation
  const negativeTransactions = primaryValues.filter(v => v < 0);
  const refundAdjustmentTotal = negativeTransactions.reduce((a, b) => a + b, 0);
  const efficiencyRatio = primarySum > 0 ? Math.round((secondarySum / primarySum) * 100) / 100 : 0;

  const businessCalculations: BusinessCalculations = {
    totalRevenue: Math.round(primarySum * 100) / 100,
    totalRevenueFormatted: formatNumberCompact(primarySum, isPrimaryCurrency),
    totalProfit: Math.round(secondarySum * 100) / 100,
    totalProfitFormatted: formatNumberCompact(secondarySum, isSecondaryCurrency),
    profitMarginPct,
    profitMarginFormatted: `${profitMarginPct.toFixed(1)}%`,
    averageOrderValue: Math.round(aov * 100) / 100,
    averageOrderValueFormatted: formatNumberCompact(aov, isPrimaryCurrency),
    topSegmentName: topSegment.category,
    topSegmentRevenue: topSegment.primarySum,
    topSegmentSharePct: topSegment.share,
    topSegmentShareFormatted: `${topSegment.share.toFixed(1)}%`,
    paretoTop20SharePct,
    paretoTop20ShareFormatted: `${paretoTop20SharePct.toFixed(1)}%`,
    periodGrowthPct,
    periodGrowthFormatted: periodGrowthPct !== null ? `${periodGrowthPct >= 0 ? '+' : ''}${periodGrowthPct.toFixed(1)}%` : 'N/A',
    refundAdjustmentCount: negativeTransactions.length,
    refundAdjustmentTotal: Math.round(refundAdjustmentTotal * 100) / 100,
    refundAdjustmentFormatted: formatNumberCompact(refundAdjustmentTotal, isPrimaryCurrency),
    efficiencyRatio,
    efficiencyRatioFormatted: `${efficiencyRatio.toFixed(2)}x`,
  };

  // Metrics Summary for Executive Dashboard
  const metrics: DashboardMetricsSummary = {
    ...
    topSegmentShareFormatted: `${topSegment.share.toFixed(1)}%`,
    paretoTop20ShareFormatted: `${paretoTop20SharePct.toFixed(1)}%`,
    periodGrowthFormatted: businessCalculations.periodGrowthFormatted,
    refundAdjustmentTotalFormatted: businessCalculations.refundAdjustmentFormatted,
    refundAdjustmentCount: negativeTransactions.length,
  };

...
export const ExplorerView: React.FC<ExplorerViewProps> = ({ profile }) => {
  const [activeMode, setActiveMode] = useState<'table' | 'sql'>('table');
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider text-blue-400 font-mono">
              Raw Record Level Inspector & SQL
            </span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-100 tracking-tight font-display">
            Data Explorer & Query Engine
          </h1>
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            Audit cell quality flags, explore raw records, or execute arbitrary SQL queries against the dataset in-memory.
          </p>
        </div>
